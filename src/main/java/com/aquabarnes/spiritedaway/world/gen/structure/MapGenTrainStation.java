...
public class MapGenTrainStation extends MapGenStructure
{
    ...
    public MapGenTrainStation()
    {
        this.structureCoords = new ChunkCoordIntPair[128];
        this.field_82671_h = 32.0D;
        this.field_82672_i = 3;
        this.BiomeList = Lists.<BiomeGenBase>newArrayList();
        ...
        {
            if (!this.BiomeList.contains(biome))
            {
    ...
    public MapGenTrainStation(Map<String, String> p_i2068_1_)
    {
        this();
    ...
    private void func_189104_c()
    {
        ...
        if (l < this.structureCoords.length)
        {
            for (int i1 = 0; i1 < this.structureCoords.length; ++i1)
            {
                ...
                BlockPos blockpos = this.worldObj.getBiomeProvider().findBiomePosition((j1 << 4) + 8, (k1 << 4) + 8, 112, this.BiomeList, random);
    ...
    {
        MapGenTrainStation.Start mapgenstronghold$start;

        for (mapgenstronghold$start = new MapGenTrainStation.Start(this.worldObj, this.rand, chunkX, chunkZ); mapgenstronghold$start.func_186161_c().isEmpty() || ((StructureStrongholdPieces.Stairs2)mapgenstronghold$start.func_186161_c().get(0)).strongholdPortalRoom == null; mapgenstronghold$start = new MapGenTrainStation.Start(this.worldObj, this.rand, chunkX, chunkZ))
        {
            ;
        }
...
//:TODO This will be the map gen to get to the Spirit Realms Dimension.